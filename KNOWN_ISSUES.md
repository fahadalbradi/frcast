...
# FIX — validator flagged date components as ungrounded numbers

**Reported:** the validator marked `16` and `2017` as ungrounded; they came from the forecast
date `2017-08-16`, not from a hallucination.

**Cause:** `_NUM_RE` scanned the raw prose, so a date string yielded its components
(`2017`, `05`, `21`, and `00`s from timestamps) as free-standing numbers.

**Fix (forecast_narrative.py only):** date-like substrings are stripped BEFORE number
extraction. Supported: `2017-08-16`, `2017-08-16 00:00:00`, `08/16/2017`, `16-08-2017`,
`2017/08/16`. They are returned in `validation["dates_skipped"]` and shown in the UI, so the
exclusion is visible, not silent.

**The validator was NOT weakened.** Only date-shaped substrings are exempt; the grounded set is
unchanged. Verified:
- prose with a date + real MAE/coverage -> all_grounded True, date listed as skipped;
- all five date formats handled;
- `"On 2017-08-16 sales will grow 23.7% and reach 5000 units"` -> date ignored, `23.7` and
  `5000` STILL flagged;
- `"Revenue will hit 2017 units"` (bare year, no date context) -> `2017` still flagged.

## Answering the second question: are 0.989 / 7.12 / 77.14 in ForecastResult?

Checked against `collect_grounded_numbers`:
- **77.14 — GROUNDED.** It is the interval coverage (0.7714) expressed as a percentage; the
  grounded set includes `coverage*100`.
- **0.989 — NOT grounded.** No such value exists in the result.
- **7.12 — NOT grounded.** No such value exists in the result.

So if the model emitted 0.989 or 7.12, they were derived/invented by the LLM and the validator
was right to flag them. That is the mechanism working as intended, not a false positive.
The confidence score, its three breakdown components, coverage, model/naive MAE/RMSE/MAPE, fold
MAEs, forecast points and the derived improvement % are all in the grounded set — anything
outside that is, by definition, not something the core computed.
