...
if result is not None:
    ...
    with tab_forecast:
        ...
        st.caption("Runs the forecast pipeline (detection → temporal prep → backtest → "
                   "intervals → report). Numbers come from the model; charts and report read "
                   "them directly.")

        raw_df = st.session_state.raw_df
        fcol1, fcol2, fcol3 = st.columns(3)
        # let the user pick the time axis and target from the raw columns
        ...
        # gap strategy is REQUIRED — no default (Step-2 decision)
        strategy = fcol3.selectbox("Gap strategy",
                                   options=["none", "ffill", "linear", "zero", "mean"],
                                   index=1,
                                   help="How to fill missing periods on the regular grid. "
                                        "ffill=carry forward, linear=interpolate, zero=fill 0, "
                                        "mean=series mean, none=leave gaps.")
