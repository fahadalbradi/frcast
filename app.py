...
def render_forecast(raw_df):
    ...
    if fr is not None:
        ...
        if rep:
            ...
            # ---- LLM narrative (LAST step; optional; validated) ----
            st.markdown("#### Narrative (OpenAI, optional)")
            st.caption("Plain-language explanation generated AFTER all numbers exist. Every "
                       "figure in it is checked against the forecast result; ungrounded numbers "
                       "are flagged.")
            nkey = st.text_input("OPENAI_API_KEY (optional, not saved)", type="password",
                                 value=os.environ.get("OPENAI_API_KEY", ""), key="fcst_key")
            if st.button("Generate narrative"):
                if nkey:
                    os.environ["OPENAI_API_KEY"] = nkey
                from prediction_engine.forecast_narrative import generate_narrative
                with st.spinner("Generating narrative..."):
                    nar = generate_narrative(fr, rep)
                if not nar["available"]:
                    st.warning(nar["error"])
                else:
                    if nar.get("narrative_status") == "UNVERIFIED":
                        st.error(nar["warning"])
                        st.markdown(f"_Unverified narrative:_\n\n{nar['narrative']}")
                    else:
                        st.success("Narrative verified — every number is grounded in the result.")
                        st.markdown(nar["narrative"])
                    _v = nar.get("validation") or {}
                    if _v.get("dates_skipped"):
                        st.caption(f"Dates treated as time labels, not checked as figures: "
                                   f"{', '.join(_v['dates_skipped'])}")
