...
if result is not None:
    ...
    tab_fp, tab_prep, tab_eval, tab_predict, tab_forecast, tab_log = st.tabs(
        ["Data Fingerprint", "Preprocessing", "Triple-Threat Evaluation", "Predict", "LLM Forecast", "Run Log"]
    )
    ...
    # ---------------- LLM Forecast tab ----------------
    with tab_forecast:
        st.markdown("**LLM Forecast** — Descriptive analysis explaining results and risks.")
        api_key_input = st.text_input("ANTHROPIC_API_KEY (optional, not saved)", type="password",
